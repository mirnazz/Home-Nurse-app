...
class NurseServiceRequestItem {
  ...
  factory NurseServiceRequestItem.fromApiJson(Map<String, dynamic> json) {
    ...
    try {
      if (dateRaw.isNotEmpty && timeRaw.isNotEmpty) {
        ...
        parsedDateTime =
            DateTime.parse('${dateRaw.split('T').first}T$normalizedTime');
  ...
  static NurseServiceRequestStatus _mapStatus(String statusRaw) {
    switch (statusRaw) {
      case 'accepted':
        return NurseServiceRequestStatus.accepted;
      case 'declined':
      case 'rejected':
        return NurseServiceRequestStatus.declined;
      case 'pending':
      default:
        return NurseServiceRequestStatus.pending;
    }
  }
